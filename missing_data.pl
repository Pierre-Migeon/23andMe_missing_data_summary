...
#@chromsomes = an array of each chromosome, in order of input, such that the order can be maintained for output without having to sort.
#####################################
#$*$*$*$*$*$*$*$*$*$*$*$*$*$*$*$*$*$
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#######################################################
...
while(<IN>)
{
	unless($_ =~ '^#') {
		...
		if((!defined($missing_by_chr{$line[1]})) || eof) {
			if (!defined($missing_by_chr{$line[1]})) { 
				$missing_by_chr{$line[1]}[3] = $line[2]; 
				push @chromosomes, $line[1];
			}
			if (!defined($missing_by_chr{$line[1]}[1])) { $missing_by_chr{$line[1]}[1] = 0; }
			if (eof) { $missing_by_chr{$line[1]}[2] = $line[2]; }
			if (defined($missing_by_chr{'last chr'}[0])) { $missing_by_chr{$missing_by_chr{'last chr'}[0]}[4] = $missing_by_chr{$missing_by_chr{'last chr'}[0]}[2] - $missing_by_chr{$missing_by_chr{'last chr'}[0]}[3]; }
		}
		if ($line[3] =~ m/--/) {
		...
		$missing_by_chr{'last chr'}[0] = $line[1];
		$total_count++;
	}
}
close (IN);
...
print $fh2 "Chromosome\tRange\tSNPs\tSNP_density\tMissing\tpercent_missing\tMissing_density\n";
foreach (@chromosomes) {
	print $fh2 "$_\t$missing_by_chr{$_}[4]\t$missing_by_chr{$_}[0]\t" . ($missing_by_chr{$_}[0] / $missing_by_chr{$_}[4]) . "\t$missing_by_chr{$_}[1]\t" . ($missing_by_chr{$_}[1] / $missing_by_chr{$_}[0]) . "\t" . ($missing_by_chr{$_}[1] / $missing_by_chr{$_}[4]) . "\n";
}
close $fh2;
