...
open (IN, '<', "./synthetic.Homo_sapiens.GRCh38.97.gtf");
while (<IN>) {
	unless ($_ =~ '^#') {
		my @line2 = split('\t', $_);
		#if it's a gene:
...
foreach (@chromosomes) {
	if (defined($gene_ranges{$_}{'unprocessed_pseudogene'})) {
		for(my $z = 0; $z < @{$gene_ranges{$_}{'unprocessed_pseudogene'}} - 1; $z += 2) {
			print $merge "$_\t$gene_ranges{$_}{'unprocessed_pseudogene'}[$z]\t$gene_ranges{$_}{'unprocessed_pseudogene'}[$z + 1]\n";
		}
	}
}
close $merge;
